...
import { handleCustomerGroupLink } from '@app/apollo/customerGroupLink';
import { clearRewardDataFromCache } from '@app/overrides/Apollo/clearRewardDataFromCache';
...
export const useAdapter = props => {
    ...
    const customerGroupLink = useMemo(
        () =>
            setContext((request, { headers }) => {
                // Return the headers to the context so httpLink can read them
                return handleCustomerGroupLink(request.operationName, headers);
            }),
        []
    );
    ...
    const apolloLink = useMemo(
        ...
        [authLink, customerGroupLink, errorLink, httpLink, magentoGqlCacheLink, mutationQueueLink, retryLink, storeLink]
    );
    ...
    const clearCacheData = useCallback(
        async (client, cacheType) => {
            const storeCode = storage.getItem('store_view_code') || STORE_VIEW_CODE || 'default';
            ...
            } else if (cacheType === 'reward') {
                await clearRewardDataFromCache(client);
            }
            ...
            for (const store of AVAILABLE_STORE_VIEWS) {
                if (store.code !== storeCode) {
                    ...
                    if (existingStorePersistor && Object.keys(existingStorePersistor).length > 0) {
                        ...
                        storeClient.persistor = isServer ? null : createCachePersistor(store.code, storeCache);
                        ...
                        if (cacheType === 'cart') {
                            ...
                            await clearRewardDataFromCache(storeClient);
                        }
                    }
                }
            }
