import { BLOG_URLS_BY_SECTION, PAGE_TYPES } from '@amasty/blog-pro/src/constants';
...
export const usePageBuilder = () => {
    ...
    const isHomePage = pathname.replace(/\/\/?$/, '') === BLOG_URLS_BY_SECTION.HOME;
    ...
    const layout = useMemo(() => {
        ...
        return getLayoutInfo(settings[`layout_${deviceType}_${layoutType}`] || settings.layout_desktop_list);
    ...
    const layoutStyle = useMemo(() => {
        ...
        const defaultAreas = layoutType === 'post' ? ['breadcrumbs'] : ['breadcrumbs', 'heading'];
        ...
        if (isMobile) {
            ...
            defaultAreas.forEach(area => {
                gridTemplateAreas += `"${hasLeftSide ? area : ''} ${area} ${hasRightSide ? area : ''}"`;
            });

            for (let i = 0; i < content.length; i += 1) {
                gridTemplateAreas += `\n"${hasLeftSide ? 'leftSide' : ''} ${content[i]} ${
                    hasRightSide ? 'rightSide' : ''
                }"`;
            }

            gridTemplateAreas += `\n"${hasLeftSide ? 'leftSide' : ''} . ${hasRightSide ? 'rightSide' : ''}"`;
        }
    ...
    const [{ drawer }, { toggleDrawer, closeDrawer: handleClose }] = useAppContext();
    ...
    useEffect(setDeviceTypeHandler, [setDeviceTypeHandler]);
