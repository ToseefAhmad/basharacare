...
export const useAddressBookPage = (props = {}) => {
    ...
    const deleteAddress = useCallback(
        ...
        [addToast, deleteCustomerAddress, formatMessage, getCustomerAddressesQuery]
    );
    ...
    const handleAddAddress = useCallback(() => {
        ...
        setFormAddress({ country_code: defaultCountry });
            ...
            if (isDialogEditMode) {
                try {
                    addToast({
                        type: 'success',
                        message: formatMessage({
                            id: 'adress.successEddited',
                            defaultMessage: 'Address successfully Edited.'
                        })
                    });
                    ...
                    await createCustomerAddress({
                        variables: {
                            address: {
                                ...
                                // Cleans up the street array when values are null or undefined
                                street: formValues.street.filter(e => e),
                                default_billing: !customerAddresses.length,
                                default_shipping: !customerAddresses.length
                            }
                        },
                        refetchQueries: [{ query: getCustomerAddressesQuery }],
                        awaitRefetchQueries: true
