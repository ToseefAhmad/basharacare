...
import { useStoreCookieStatus } from '@app/hooks/useStoreCookieStatus';
import { clearCacheData } from '@app/util/clearCacheData';
...
export const useStoreSwitcher = (props = {}) => {
    ...
    const {
        ...
        expanded: languageIsOpen,
        setExpanded: setLanguageIsOpen,
        triggerRef: languageTriggerRef
    } = useDropdown();

    const { setStoreCookieStatus } = useStoreCookieStatus();
    ...
    const handleSwitchStore = useCallback(
        ...
        storeCode => {
            ...
            setStoreCookieStatus(storeCode, availableStores.get(storeCode).locale);
            ...
            // Clear other store local storage
            clearCacheData(storeCode);
            ...
            if (process.env.USE_STORE_CODE_IN_URL === 'true') {
                ...
                if (pathName !== '' && pathName !== '/') {
                    ...
                    if (availableStores.has(pathStoreCode) && availableStores.get(pathStoreCode).isCurrent) {
                        const newPath = `${pathName.replace(`/${pathStoreCode}`, `/${storeCode}`)}${params}`;
        ...
        [availableStores, getPathname, setStoreCookieStatus]
    );
    ...
    const handleLanguageTriggerClick = useCallback(() => {
        // Toggle Languages Stores Menu.
        setLanguageIsOpen(isOpen => !isOpen);
    }, [setLanguageIsOpen]);
