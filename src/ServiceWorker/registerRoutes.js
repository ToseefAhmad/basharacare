...
import { isResizedImage, findSameImage, createImageCacheHandler } from './Utilities/imageCacheHandler';
...
export default function() {
    ...
    registerRoute(isResizedImage, ({ url, request, event }) => {
        const sameImagePromise = findSameImage(url, request);
        event.waitUntil(sameImagePromise);
        return sameImagePromise.then(response => response || imageCacheHandler.handle({ request, event }));
    });
